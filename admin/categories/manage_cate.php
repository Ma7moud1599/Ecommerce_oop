...
class Manage_Cate
{
    ...
    public function get_data($sort)
    {
        ...
        $stmt = $database->con->prepare("SELECT * FROM categories ORDER BY ID $sort");
    ...
    public function errors($Name, $imageName, $imageExtension, $imageAllowedExtension, $imageSize)
    {
        $formsError = array();
        if (strlen($Name) < 4) {
            $formsError[] = 'Category Can\'t be Less than 4 Character';
        }

        if (strlen($Name) > 20) {
            $formsError[] = 'Category Can\'t be More Than 20 Character';
        }
        ...
        foreach ($formsError as $error) {
            ...
            $func = new Func;
            $func->redirectToHome($massege, 3, 'categories.php');
        }
    }
